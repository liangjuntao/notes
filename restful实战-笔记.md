...
rest式web服务通过资源来暴露数据和算法。  
如：地图上符合搜索条件的地点。而不是在地图上搜索地点！  

查询变量  /path/place？show==xxx&show=xxx  
路径变量  /path/place/china/hangzhou  
...
- 设计步骤：  
	1. 规划数据集  
	2. 把数据集划分为资源  
	3. 对于其中的每种资源，用uri为该资源命名  
	4. 暴露一个统一接口的子集  
	5. 设计来自客户端的表示  
	6. 设计发给客户端的表示  
	7. 用超链接和表单把该资源与已有资源进行关联  
	8. 考虑有哪些典型的事件经过  
	9. 考虑可能出现哪些错误情况  
- 数据规划的时候如何决定暴露资源

后边的暂时不看了，对rest这个感念有初步的感受。已经达到我的目的了。工作中有机会，再学习吧。
